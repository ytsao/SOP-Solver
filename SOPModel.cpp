...
SOPRange SOPModel::addLe(SOPExpression &con, double rhs, std::string con_name){
    ...
    SOPVariables basic = SOPVariables("x"+std::to_string(this->get_num_cols()+this->get_num_rows()+1), 'R'
...
void SOPModel::set_eta(int leaving_index, std::vector<double> delta_x_B){
    ...
    for(int i = 0; i < this->num_rows; ++i){
        double value=0;
        if (i != leaving_index) value = -(delta_x_B[i] / delta_x_B[leaving_index]);
        else value = 1/delta_x_B[leaving_index];
